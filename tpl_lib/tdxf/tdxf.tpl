...
function t_cut(x, y, z, step,vdia, incremental){
  ...
  if (incremental){
    ...
    stepcount=Math.floor(Math.abs(cutlength/step));
	console.log('Setpcount: '+stepcount);
    if ((cutlength%step)>0)stepcount++;
	console.log('Setpcount: '+stepcount);
	if (stepcount>0){
	cut({
...
module.exports = extend({
  ...
  set_virtdia:function(d) {
	virtdia=d;
  },
  
  set_stepfeed: function(s){
	stepfeed=s;
  },
  ...
  tabbed_cut: function(x, y) {
    ...
    */
	...
	console.log('bgb');
	console.log(x+' '+y);
	t_cut(x,y,p.z,stepfeed,virtdia,false);
  },
  ...
  arc_cut: function(a) {
	...
	t_arc(offset.x, offset.y,0,this.arc_angle(a),XY,stepfeed,virtdia);
  },
  ...
  layer_cut: function(layer, zSafe, zCut, res) {
    ...
    while (layer.length) {
      ...
      cut({z: zCut});
	  //t_cut(position.x,position.y,zCut,0.05,1,false);
      this.tabbed_cut(v.x, v.y);
